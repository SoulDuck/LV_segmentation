...
# read the proper data set
        images, labels = nn.inputs(batch_size=FLAGS.batch_size,num_epochs=1, filename=evalfile)
...
#                              label_sum, example_sum)
                    if __debug__ == True:
                        print 'count', count
                        images_ , labels_ , logits_=sess.run([images , labels, logits])
                        images_ , labels_, logtis_=map(np.squeeze , [images_ , labels_ , logits_])
